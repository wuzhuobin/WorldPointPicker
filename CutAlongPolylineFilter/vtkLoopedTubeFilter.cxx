...
vtkStandardNewMacro(vtkLoopedTubeFilter);

int vtkLoopedTubeFilter::RequestData(vtkInformation* request,
	vtkInformationVector** inputVector,
	vtkInformationVector* outputVector)
{
	/// Creates a tube first
	...
	if (input->GetNumberOfCells() > 1) {
		vtkWarningMacro(<< "Input must only has one poly-line type cell!");
		return 0;
	}

	if (input->GetCell(0)->GetCellType() != VTK_POLY_LINE) {
		vtkWarningMacro(<< "Input cell type must be VTK_POLY_LINE");
		return 0;
	}

	if (input->GetCell(0)->GetNumberOfPoints() != input->GetNumberOfPoints() + 1 ||
		input->GetCell(0)->GetPointId(input->GetNumberOfPoints()) != 0) {
		vtkWarningMacro(<< "Input dosen't seems to be a loop! Maybe your input is not sorted by point Ids");
		return 0;
	}
	...
	inLines->GetNextCell(npts, pts) && !abort; inCellId++)
	{
		...
		if (!this->GeneratePointsForLoop(offset, npts, pts, inPts, newPts, pd, outPD,
		...
		//
		this->GenerateStripsForLoop(offset, npts, pts, inCellId, cd, outCD, newStrips);
...
void vtkLoopedTubeFilter::GenerateStripsForLoop(vtkIdType offset, vtkIdType npts, vtkIdType* vtkNotUsed(pts), vtkIdType inCellId, vtkCellData *cd, vtkCellData *outCD, vtkCellArray *newStrips)
{
	...
	if (this->SidesShareVertices)
	{
		...
		k += this->OnRatio)
		{
			...
			for (i = 0; i < npts - 1; i++)
			{
				...
				//char msg[200];
				//sprintf_s(msg, "[%i, %i, %i]->{%i, %i}\n", i1, i2, i3, i2+i3, i1+i3);
				//cout << msg;
			}
			newStrips->InsertCellPoint(offset + i2);
			newStrips->InsertCellPoint(offset + i1);
...
int vtkLoopedTubeFilter::GeneratePointsForLoop(vtkIdType offset, vtkIdType npts, vtkIdType *pts, vtkPoints *inPts, vtkPoints *newPts, vtkPointData *pd, vtkPointData *outPD, vtkFloatArray *newNormals, vtkDataArray *inScalars, double range[2], vtkDataArray *inVectors, double maxSpeed, vtkDataArray *inNormals)
{
	vtkIdType j;
	int i, k;
	double p[3];
